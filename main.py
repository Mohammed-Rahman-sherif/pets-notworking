...
def main():
    ...
    # Initialize wandb
    wandb.init(
        # project="ICML_OxfordPets",
        # name=f"{args.shots}_shot_b16",
        config=vars(args)  # if args is an argparse.Namespace
    )
    config = wandb.config
    ...
    val_loader = build_data_loader(data_source=dataset.val, batch_size=64, is_train=False, tfm=val_transform)
    test_loader = build_data_loader(data_source=dataset.test, batch_size=64, is_train=False, tfm=val_transform)
